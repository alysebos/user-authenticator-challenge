...
function gateKeeper(req, res, next) {
  ...
  const userCredentialsString = req.get('x-username-and-password');
  const {user, pass} = queryString.parse(userCredentialsString);  
  
  req.user = USERS.find(entry => entry.userName === user && entry.password === pass);
